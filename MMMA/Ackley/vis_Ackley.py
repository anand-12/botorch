...
file_names = [os.path.basename(file_path)[:20] + '...' for file_path in file_paths]
...
for i, metric in enumerate(metric_names):
    ...
    if metric in ["Simple Regret", "Cumulative Regret"]:
        ax.set_title(f'Log {metric} Across 25 Experiments')
        ax.set_yscale('log')
    else:
        ax.set_title(f'Median {metric} Across 25 Experiments')
